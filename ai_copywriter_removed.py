...
명사 위주로 간결하게 작성해주세요.
이모지를 적절하게 섞어주세요.
키워드가 주어질 경우,반드시 키워드중 하나를 포함해야합니다. 
...
def print_streaming_respose(response):
  message =""
  pleaceholder =st.empty()
  for chunk in response:
    delta = chunk.choices[0].delta
    if delta.content:
      message += delta.content
      print(delta.content,end="")
      pleaceholder.markdown(message+"▌")
  pleaceholder.markdown(message)
with st.form("from"):
  ...
  with col2:
    max_length = st.number_input("최대 단어수",min_value=5,max_value=20,step=1,value=10)
  with col3:
    num = st.number_input("생성할 문구 수",min_value=1,max_value=10,step=1,value=5)
  desc =st.text_input(label="제품 간단 정보 (필수)", value=example["desc"] if auto_complete else "",
                         placeholder=example["desc"])
  st.text("포함할 키워드(최대 3개까지 허용)")
  col1,col2,col3 =st.columns(3)
  with col1:
    keyword_one = st.text_input(label="keyword_1",label_visibility="collapsed",
                                 value=example["keywords"][0] if auto_complete else "",
                         placeholder=example["keywords"][0])
  with col2:
    keyword_two = st.text_input(label="keyword_2",label_visibility="collapsed",
                                 value=example["keywords"][1] if auto_complete else "",
                         placeholder=example["keywords"][1])
  with col3:
    keyword_three = st.text_input(label="keyword_3",label_visibility="collapsed",
...
if submit:
  if not name:
    ...
    print_streaming_respose(response)
